...
public class S1600 {
    class ThroneInheritance {
        ...
        public ThroneInheritance(String kingName) {
            ...
            nodesMap.put(kingName, new Node(kingName));
        }

        public void birth(String parentName, String childName) {
            Node child = new Node(childName);
            nodesMap.get(parentName).childs.add(child);
            nodesMap.put(childName, child);
        }

        public void death(String name) {
            nodesMap.get(name).death = true;
        }

        public List<String> getInheritanceOrder() {
            return getInheritanceOrder(kingName);
        }

        private List<String> getInheritanceOrder(String name){
            List<String> res = new ArrayList<>();
            Node parent = nodesMap.get(name);
            if(!parent.death) res.add(name);
            for(Node child : parent.childs){
                res.addAll(getInheritanceOrder(child.name));
            }
            return res;
        }
    }
